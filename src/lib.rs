...
/// Provides the `BincodeBuilder` type which acts as a sink to (de)serialize a `&[u8]` as bincode.
#[cfg(feature = "bincode")]
mod bincode;
#[cfg(feature = "bincode")]
pub use crate::bincode::Builder as BincodeBuilder;

/// Provides the `BufferedBuilder` type which helps build `BufReader` and `BufWriter` instances.
mod buffered;
pub use buffered::Builder as BufferedBuilder;

/// Provides the `FileBuilder` type which acts as a source to read from and write to a file.
mod file;
pub use file::Builder as FileBuilder;

/// Provides several wrapper types around the streaming compression algorithms provided by the flate2 crate.
#[cfg(feature = "flate2")]
mod flate2;
#[cfg(feature = "flate2")]
pub use crate::flate2::{CompressionBuilder, Constructor, CrcBuilder};

/// Provides the `ProcessBuilder` type which acts as a source to read from stdout and write to stdin of a running process.
mod process;
pub use process::Builder as ProcessBuilder;

/// Provides several wrapper types around the streaming cipher algorithms provided by the flate2 crate.
...
/// Provides the `TcpStreamBuilder` type which acts as a source to read from and write to a TCP stream.
mod tcp_stream;
pub use tcp_stream::Builder as TcpStreamBuilder;

/// Provides the `VecBuilder` type which acts as a source to read from and write to a memory buffer.
mod vec;
    ...
    Self::Writer: std::io::Write,
{
    ...
    ) -> CompressionBuilder<Self, flate2::Deflate> {
    ...
    #[cfg(feature = "flate2")]
    fn gz(self, compression: ::flate2::Compression) -> CompressionBuilder<Self, flate2::Gz> {
    ...
    #[cfg(feature = "flate2")]
    fn zlib(self, compression: ::flate2::Compression) -> CompressionBuilder<Self, flate2::Zlib> {
